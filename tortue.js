...
function Tortue(id,nom_canvas, dessin, logo) { /******************************/
    ...
    this.bulle = 0;
    this.text='';
    this.temps_affichage = 10000; // affichage des bulles : 10s. par d√©faut (en milliemes de secondes)
    this.timer=null;
...
/* Ces points servent √† la gestion des collisions                            */
    ...
    switch (this.ordre.procedure.code) {
        ...
        case 'AV'       :   direction = ((this.cap+90) * (Math.PI / 180));
                            do {
                                ...
                                dx = this.posx + dx;
                                dy = this.posy + dy;  
                                if (this.collision(dx,dy)) { // DÈplacement incomplet on renvoie la diffÈrence
                                    ...
                                    if (this.param_tr[0]<=0) { // DÈplacement complet
